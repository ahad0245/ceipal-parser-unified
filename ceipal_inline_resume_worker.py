...
def get_token_value(data, *keys):
    """Return the first token value found from a CEIPAL response."""
...
def set_access_token_expiry():
    """
    CEIPAL access tokens are valid for 1 hour.
    We refresh a little early so a token does not expire in the middle of a request.
    """
    global token_expiry
...
def create_new_auth_token():
    """Login with CEIPAL credentials and store both access and refresh tokens."""
...
def refresh_access_token():
    """
    Refresh the CEIPAL access token.

    CEIPAL ATS V2 refresh endpoint returns a new access token.
    Some accounts may also return a rotated refresh token, so we keep it if provided.
    """
    global access_token, refresh_token
    ...
    print("Refreshing CEIPAL access token...")
...
def get_access_token(force_refresh=False):
    """
    Return a valid CEIPAL access token.

    Normal flow:
    1. Reuse current access token while it is still valid.
    2. Refresh it when it is near expiry.
    3. If refresh fails or no token exists, create a new auth token.
    """
    ...
    if access_token and refresh_token:
        try:
            ...
            print(f"CEIPAL refresh token failed. Falling back to new auth token. Error: {exc}")
...
def upsert_candidate(candidate, page_number=None):
    ...
    initial_resume_status = "pending" if candidate.get("resume_token") else "missing_token"
    if candidate.get("resume_token"):
        initial_parsed_status = "pending"
    else:
        initial_parsed_status = "missing_token"
    ...
    candidates_col.update_one(
        {"applicant_id": applicant_id},
        {
            ...
            "$setOnInsert": {
                ...
                "resume_download_status": initial_resume_status,
                "resume_download_error": None,
                "parsed_status": initial_parsed_status,
                "parsed_error": None,
            },
        },
        upsert=True,
...
def save_parsed_resume(candidate, resume_text):
    ...
    if not candidate_id:
        raise RuntimeError("Cannot save parsed resume without candidate Mongo _id.")
